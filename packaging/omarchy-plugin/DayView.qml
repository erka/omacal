...
// Single-day content in the panel scroll area; layout is shared with macOS.
Item {
  id: root
  ...
  property Flickable scrollView: null
  implicitHeight: content.implicitHeight
  property int cursor: -1
  function reveal(gridY, inset) {
    if (!scrollView) return
    var y = grid.mapToItem(scrollView.contentItem, 0, gridY).y
    scrollView.contentY = Math.max(0, Math.min(scrollView.contentHeight - scrollView.height, y - inset))
  }
  function moveCursor(dy) {
    if (!rows.length) return
    cursor = Math.max(0, Math.min(rows.length - 1, cursor + dy))
    reveal(rows[cursor].top * grid.height, Style.space(40))
  }
  ...
  readonly property real gridHeight: Style.space((range.end - range.start) / 60000)
  FontMetrics { id: titleMetrics; font.family: root.fontFamily; font.pixelSize: Style.font.bodySmall }
  readonly property real minimumEventHeight: Math.max(titleMetrics.height + Style.space(6), Style.space(22))
  readonly property var rows: day ? Timeline.layout(day.events, range.start, range.end, minimumEventHeight / gridHeight * (range.end - range.start)) : []
  ...
  function scrollToNow() { reveal(fraction * grid.height, Style.space(80)) }
    ...
    Item {
      ...
      Repeater {
        ...
        delegate: Rectangle {
          ...
          height: modelData.height * grid.height
